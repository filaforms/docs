...
Copy the **Signing secret** (`whsec_...`) to your `.env` as `STRIPE_CONNECT_WEBHOOK_SECRET`.
::
...
|----------|---------|-------------|
| `STRIPE_PLATFORM_SECRET_KEY` | -- | Platform account secret key (`sk_test_` or `sk_live_`) |
| `STRIPE_PLATFORM_PUBLISHABLE_KEY` | -- | Platform account publishable key (`pk_test_` or `pk_live_`) |
| `STRIPE_CONNECT_WEBHOOK_SECRET` | -- | Webhook signing secret (`whsec_...`) |
| `STRIPE_APPLICATION_FEE_PERCENT` | `2` | Platform fee percentage per transaction |
| `STRIPE_DEFAULT_CURRENCY` | `USD` | Default currency (ISO 4217) |
